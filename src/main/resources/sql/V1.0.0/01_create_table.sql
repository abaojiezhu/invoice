...
CREATE TABLE `app_token` (
  `ID` char(32) NOT NULL COMMENT 'token',
  `USER_ID` char(32) DEFAULT NULL COMMENT '用户ID',
  `CREATED_DT` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='App Token';

DROP TABLE IF EXISTS `sys_encrypt_key`;
CREATE TABLE `sys_encrypt_key` (
    `APP_ID` char(32) NOT NULL COMMENT '每个公司对应一个唯一的id标识',
	`ENCRYPT_KEY` char(128) DEFAULT NULL COMMENT '密钥',
	...
	`UPDATED_USER_ID` char(32) DEFAULT NULL COMMENT '更新人',
    PRIMARY KEY (`APP_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='appid和密匙对应表';
